...
OUTPUT_DIR = "outputs_resnet_with_skip/plots"
...
def plot_architecture_table():
    ...
    title = "Table 1. ResNet WITH Skip Connections — Architecture for SVHN (32×32 input)"

    columns = ["Layer Name", "Output Size", "ResNet-20 (Ours)", "Operation"]
    rows = [
        ["conv1",       "32×32",  "3×3, 64, stride 1",                         "Conv + BN + ReLU"],
        ["stage1_b1",   "32×32",  "[3×3, 64] × 1\n[3×3, 64]",                  "BasicBlock (skip)"],
        ["stage1_b2",   "32×32",  "[3×3, 64] × 1\n[3×3, 64]",                  "BasicBlock (skip)"],
        ["stage1_b3",   "32×32",  "[3×3, 64] × 1\n[3×3, 64]",                  "BasicBlock (skip)"],
        ["stage2_b1",   "16×16",  "[3×3, 128, stride 2] × 1\n[3×3, 128]",      "BasicBlock (proj skip)"],
        ["stage2_b2",   "16×16",  "[3×3, 128] × 1\n[3×3, 128]",                "BasicBlock (skip)"],
        ["stage2_b3",   "16×16",  "[3×3, 128] × 1\n[3×3, 128]",                "BasicBlock (skip)"],
        ["stage3_b1",   "8×8",    "[3×3, 256, stride 2] × 1\n[3×3, 256]",      "BasicBlock (proj skip)"],
        ["stage3_b2",   "8×8",    "[3×3, 256] × 1\n[3×3, 256]",                "BasicBlock (skip)"],
    ...
    row_colors = [
        other_color,   # conv1
        stage1_color, stage1_color, stage1_color,   # stage1
        stage2_color, stage2_color, stage2_color,   # stage2
        stage3_color, stage3_color, stage3_color,   # stage3
        other_color,   # avgpool
        other_color,   # fc
    ]
    ...
    ax.text(0.99, 0.01,
            "Total Parameters: 2,784,202   |   Trainable: 2,784,202\n"
            "FLOPs (approx): ~180M   |   Input: 3×32×32",
...
def plot_performance_table():
    ...
    row_colors = [
        ...
        [ref_color]*8,
        [ref_color]*8,
        [ours_color]*8,   # our model highlighted
    ]
    ...
    # Bold test acc column
    for i in range(1, len(rows) + 1):
        table[i, 5].set_text_props(fontweight="bold")

    legend_elements = [
        mpatches.Patch(facecolor=ours_color, edgecolor="gray", label="★ Our Experiment (ResNet WITH Skip)"),
...
def plot_training_metrics_table():
    ...
    title = "Table 3. Epoch-by-Epoch Training Metrics — ResNet WITH Skip Connections"
    ...
    # Gradient norms for first and last tracked layer
    ...
    # Color rows by test accuracy quartiles
    test_acc_arr = np.array(test_acc)
    best_epoch   = int(np.argmax(test_acc_arr))

    def row_color(i):
        if i == best_epoch:
            return ["#a9dfbf"] * len(columns)   # green = best epoch
    ...
    ax.text(0.99, 0.995,
            f"Key Insight (Skip Connections):\n"
            f"Mean Grad Ratio (first/last): {mean_ratio:.2f}\n"
            f"→ Ratio > 0.1 confirms no vanishing gradient\n"
            f"→ All layers receive non-zero weight updates",
            transform=ax.transAxes, fontsize=9,
            ha="right", va="top",
...
def plot_gradient_summary_table():
    ...
    title = ("Table 4. Gradient Norm Summary (L2) — Key Epochs\n"
             ...
             "Consistent norms across depth = healthy gradient flow")

    layer_names = list(history["epoch_grad_norms"].keys())
    key_epochs  = [0, 4, 9, 14, 19]   # epochs 1, 5, 10, 15, 20
    ...
    for layer in layer_names:
        ...
        # Trend: compare first vs last epoch
        trend = "↓ Decreasing" if norms[-1] < norms[0] else "↑ Increasing"
        row.append(trend)
        rows.append(row)

    # Color by gradient magnitude (relative to max in each column)
    def get_cell_color(val_str, col_max, col_min):
        try:
            val = float(val_str)
            norm = (val - col_min) / (col_max - col_min + 1e-10)
            r = 1.0
            g = 0.3 + 0.7 * norm
            b = 0.3 + 0.7 * norm
            return (r, g, b)
...
# ============================================================
print("=" * 60)
print("Generating Tables from Training Results")
print("=" * 60)
...
print("\nFiles generated:")
print("  table1_architecture.png   — Layer-by-layer architecture (like paper Table 1)")
print("  table2_performance.png    — Accuracy comparison vs literature (like paper Table 3)")
